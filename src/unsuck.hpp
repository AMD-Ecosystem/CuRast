...
struct AsyncFileReader{
	shared_ptr<jthread> thread = nullptr;
	string path;
	shared_ptr<Buffer> buffer = nullptr;

	shared_ptr<Buffer> await(){
		thread->join();
...
inline shared_ptr<AsyncFileReader> readBinaryFileAsync(string path) {

	shared_ptr<AsyncFileReader> reader = make_shared<AsyncFileReader>();

	shared_ptr<jthread> thread = make_shared<jthread>([reader, path](){
		auto file = fopen(path.c_str(), "rb");
		auto size = fs::file_size(path);
...
inline void onTypeMatch(HAVE value, std::function<void(WANT)> callback){
	bool matches = dynamic_cast<WANT>(value) != nullptr;

	if(matches){
		callback(dynamic_cast<WANT>(value));
	}
}

inline vector<string> listFiles(string dir){
